...
describe('CreateStore:', () => {
  ...
  test('should call subscribe function', () => {
    ...
    store.dispatch({type: 'ADD'})
    expect(handler).toHaveBeenCalled() // вызывался
    expect(handler).toHaveBeenCalledWith(store.getState()) // вызывался с store.getState()
  })
  test('should NOT call sub if unsubscribe', () => {
    const unsub = store.subscribe(handler)
    unsub.unsubscribe()
    store.dispatch({type: 'ADD'})
    expect(handler).not.toHaveBeenCalled() // вызывался
  })
  test('async', () => {
